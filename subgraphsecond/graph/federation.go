...
func (ec *executionContext) resolveEntity(
	...
	case "Account":
		...
		case "findAccountByAccountReferenceID":
			id0, err := ec.unmarshalNString2string(ctx, rep["accountReferenceId"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findAccountByAccountReferenceID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindAccountByAccountReferenceID(ctx, id0)
			if err != nil {
	...
	case "Card":
		resolverName, err := entityResolverNameForCard(ctx, rep)
		if err != nil {
			return nil, fmt.Errorf(`finding resolver for Entity "Card": %w`, err)
		}
		switch resolverName {

		case "findCardByCardReferenceID":
			id0, err := ec.unmarshalNString2string(ctx, rep["cardReferenceId"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findCardByCardReferenceID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindCardByCardReferenceID(ctx, id0)
			if err != nil {
				return nil, fmt.Errorf(`resolving Entity "Card": %w`, err)
			}
	...
	case "Transaction":
		...
		case "findTransactionByTransactionID":
			id0, err := ec.unmarshalNString2string(ctx, rep["transactionId"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findTransactionByTransactionID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindTransactionByTransactionID(ctx, id0)
			if err != nil {
...
func entityResolverNameForAccount(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		allNull := true
		m = rep
		val, ok = m["accountReferenceId"]
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"accountReferenceId\" for Account", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		return "findAccountByAccountReferenceID", nil
	}
	return "", fmt.Errorf("%w for Account due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}

func entityResolverNameForCard(ctx context.Context, rep EntityRepresentation) (string, error) {
	// we collect errors because a later entity resolver may work fine
	// when an entity has multiple keys
	entityResolverErrs := []error{}
	for {
		...
		allNull := true
		m = rep
		val, ok = m["cardReferenceId"]
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"cardReferenceId\" for Card", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		if allNull {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to all null value KeyFields for Card", ErrTypeNotFound))
			break
		}
		return "findCardByCardReferenceID", nil
	}
	return "", fmt.Errorf("%w for Card due to %v", ErrTypeNotFound,
		errors.Join(entityResolverErrs...).Error())
}
...
func entityResolverNameForTransaction(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"transactionId\" for Transaction", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		return "findTransactionByTransactionID", nil
	}
