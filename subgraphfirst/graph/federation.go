...
func (ec *executionContext) resolveEntity(
	...
	case "Customer":
		...
		case "findCustomerByCustomerReferenceID":
			id0, err := ec.unmarshalNString2string(ctx, rep["customerReferenceId"])
			if err != nil {
				return nil, fmt.Errorf(`unmarshalling param 0 for findCustomerByCustomerReferenceID(): %w`, err)
			}
			entity, err := ec.resolvers.Entity().FindCustomerByCustomerReferenceID(ctx, id0)
			if err != nil {
...
func entityResolverNameForCustomer(ctx context.Context, rep EntityRepresentation) (string, error) {
	...
	for {
		...
		allNull := true
		m = rep
		val, ok = m["customerReferenceId"]
		if !ok {
			entityResolverErrs = append(entityResolverErrs,
				fmt.Errorf("%w due to missing Key Field \"customerReferenceId\" for Customer", ErrTypeNotFound))
			break
		}
		if allNull {
			allNull = val == nil
		...
		return "findCustomerByCustomerReferenceID", nil
	}
